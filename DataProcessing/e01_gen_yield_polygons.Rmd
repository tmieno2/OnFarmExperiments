...
yield_trd <-
  ...
  st_join(yield_grouped, trd_polygons) %>%
  #* order by yield id
  arrange(yield_id) %>%
  data.table() %>%
  .[, .(yield_id, group, treat_id, Trt, Rep, PlotNum, yield_vol, geometry)] %>%
