...
zero_tgt_polygons <-
  ...
  st_buffer(actual_machine_width / 8)
...
#   geom_sf(data = points_filling$seq_points[[2]]) +
#   geom_sf(data = points_filling$origin[[2]], color = "red") +
#   geom_sf(data = points_filling$int_seq_rx[[2]], color = "blue")
...
points_filling <-
  ...
  mutate(seq_points = list(
    make_seq_points(origin, dir_vect, length)
  )) %>%
  ...
  data.table() %>%
  .[, .SD[which.max(int_nums)], by = rx_id] %>%
  rowwise() %>%
  ...
  mutate(points_to_add = list(
    mutate(points_to_add, group = paste0("m_", rx_id), new_group = paste0("m_", rx_id))
  )) %>%
  pull(points_to_add) %>%
  rbindlist() %>%
  .[, input_rate := 0] %>%
...
```{r }
if (is_aa_input_polygon) { #* if as-applied rate is in polygon

  aa_input_sf <-
    aa_input_reduced_v %>%
    dplyr::select(-geometry) %>%
    left_join(aa_input_poly, , by = "aa_id") %>%
    rename(aa_input_id = aa_id) %>%
    st_as_sf() %>%
    st_make_valid()
} else {
...
here(
  ...
  paste0("aa-", input_type, "-", input_form, "-", input_id, ".rds")
) %>%
  saveRDS(aa_input_sf, .)
```
