...
## Flag outlier input data points
...
aa_input_grouped <- 
  flag_bad_points(
    ...
    var_name = "input_rate", 
    sd_factor = sd_factor,
    upper = TRUE # only the high end flagged
  )  

```

### Input distribution before and after cleaning
...
```{r }
aa_input_grouped %>% 
  filter(ol_input_rate == 0) %>% 
  pull(input_rate) %>% 
  hist()
```

### Which data points were flagged as outliers (right image)?
...
tm_after <- 
  tm_shape(filter(aa_input_grouped, ol_input_rate == 1)) +
  tm_symbols(col = "input_rate", size = 0.05) +
  tm_layout_to_add  
