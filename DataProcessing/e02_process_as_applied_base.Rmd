...
aa_input_trd <-
  ...
  st_join(aa_input_grouped, trd_polygons) %>%
  #* order by yield id
  arrange(id) %>%
  data.table() %>%
  .[, .(id, group, Trt, Rep, input_rate, geometry)] %>%
  #* flag outliers by treatment block
  nest_by(Trt, Rep, group) %>%
  mutate(median_input_rate =
  ...
  unnest() %>%
  data.table() %>%
  .[, .(id, Trt, Rep, ol_input_rate)]

#* join with aa_input_grouped
aa_input_grouped <-
  left_join(aa_input_grouped, aa_input_trd, by = "id")
```
