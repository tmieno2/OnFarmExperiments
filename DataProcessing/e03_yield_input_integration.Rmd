...
### Match treatment blocks and yield data
...
yield_trd <-
  st_intersection(yield_point, trd_polygons) %>%
  arrange(yield_id) %>%
  # mutate(int_area = as.numeric(st_area(.))) %>%
  data.table() %>%
  .[, .(yield_id, Trt, Rep, yield_vol, geometry)] %>%
 #=== flag outliers ===#
  nest_by(Trt, Rep) %>%
  mutate(data = list(
    flag_bad_points(
  ...
  unnest() %>%
  data.table() %>%
  setnames("ol_yield_vol", "ol_yield_by_trt_block") %>%
  .[, .(yield_id, yield_vol, Trt, Rep, ol_yield_by_trt_block)]
...
viz_outliers <-
  ...
  mutate(viz = list(
    ggplot() +
      geom_histogram(
        data = data.table(data)[ol_yield_by_trt_block == 0, ],
        aes(x = yield_vol),
        fill = "blue",
        col = "black",
        binwidth = 5,
      ...
      geom_histogram(
        data = data.table(data)[ol_yield_by_trt_block == 1, ],
        aes(x = yield_vol),
        fill = "red",
        col = "black",
        binwidth = 5,
...
yield_input_data <-
  ...
  #=== join with trial design data ===#
  left_join(., yield_trd[, .(yield_id, Trt, Rep, ol_yield_by_trt_block)], by = "yield_id") 
  #=== join with commercial Rx data ===#
  # left_join(., cm_rx_data, by = "yield_id")
